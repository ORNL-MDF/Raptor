...
from raptor.core import is_inside, compute_melt_mask, compute_melt_mask_implicit
...
def sample_melt_pool():
    ...
    width_osc = np.array(
        [[0.0001, 0.0, 0.0], [0.00002, 5.0, 0.0], [0.00001, 10.0, 0.0]],
        dtype=np.float64,
    )

    depth_osc = np.array(
        [[0.00008, 0.0, 0.0], [0.00001, 5.0, 0.0], [0.00001, 10.0, 0.0]],
        dtype=np.float64,
    )

    height_osc = np.array(
        [[0.00006, 0.0, 0.0], [0.00001, 5.0, 0.0], [0.00001, 10.0, 0.0]],
        dtype=np.float64,
    )
...
def sample_path_vector():
    ...
    path_vec = PathVector(
        start_point=start, end_point=end, start_time=0.0, end_time=0.001
    )
...
def minimal_voxels():
    ...
    return np.array(
        [[0.0, 0.0, 0.0], [0.0005, 0.0, 0.0], [0.001, 0.0, 0.0]], dtype=np.float64
    )
...
class TestComputeMeltMask:
    ...
    def test_compute_melt_mask_basic(
        self, minimal_voxels, sample_melt_pool, sample_path_vectors
    ):
        ...
        result = compute_melt_mask(
            minimal_voxels, sample_melt_pool, sample_path_vectors
        )
    ...
    def test_compute_melt_mask_unpacking(
        self, minimal_voxels, sample_melt_pool, sample_path_vectors
    ):
    ...
    def test_compute_melt_mask_single_voxel(
        self, sample_melt_pool, sample_path_vectors
    ):
    ...
    def test_compute_melt_mask_single_path_vector(
        self, minimal_voxels, sample_melt_pool, sample_path_vector
    ):
        """Test with a single path vector."""
        sample_path_vector.set_melt_pool_properties(sample_melt_pool)

        result = compute_melt_mask(
            minimal_voxels, sample_melt_pool, [sample_path_vector]
        )

        assert result.shape[0] == minimal_voxels.shape[0]

    def test_compute_melt_mask_output_shape(
        self, sample_voxels, sample_melt_pool, sample_path_vectors
    ):
    ...
    def test_compute_melt_mask_dtype(
        self, minimal_voxels, sample_melt_pool, sample_path_vectors
    ):
...
class TestComputeMeltMaskImplicit:
    ...
    def test_compute_melt_mask_implicit_basic(
        self, minimal_voxels, sample_melt_pool, sample_path_vectors
    ):
...
class TestCoreIntegration:
    ...
    def test_is_inside_with_compute_melt_mask(
        self, minimal_voxels, sample_melt_pool, sample_path_vectors
    ):
