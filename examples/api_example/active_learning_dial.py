...
# USER PARAMETERS
# -----------------------------------------------------------------------------
INITIAL_BOUNDS = [[60e-6, 200e-6]]  # Hatch spacing in meters
UNIT_BOUNDS = [[0.0, 1.0]]
...
def run_raptor_step(
    ...
    voxel_resolution_m: float = 2.5e-6,
    ...
    print("Hatch spacing: ", hatch_spacing, "Mean pore size: ", mean)
    return mean
...
def get_raptor_data_point(x_suggested):
    """
    Runs the suggested point plus a few local variations to account for simulation noise.
    """
    ensemble_xs = [x_suggested]

    span = INITIAL_BOUNDS[0][1] - INITIAL_BOUNDS[0][0]
    for _ in range(ENSEMBLE_SIZE - 1):
        delta = (random.random() - 0.5) * 2 * LOCAL_BOX_FRAC * span
        new_x = np.clip(
            x_suggested[0] + delta, INITIAL_BOUNDS[0][0], INITIAL_BOUNDS[0][1]
        )
        ensemble_xs.append([new_x])

    results_y = []
    for x_vec in ensemble_xs:
        y = run_raptor_step(x_vec[0])
...
def plot_1d_surrogate(mean_grid, variance_grid, dataset_x, dataset_y):
    ...
    hx = np.array(HISTORY_X).flatten() * 1e6
    ...
    plt.scatter(
        np.array(dataset_x).flatten() * 1e6,
        dataset_y,
        c="red",
        marker="x",
...
class ActiveLearningOrchestrator:
    def __init__(self, service_destination: str):
        ...
        self.dataset_x = qmc.scale(
            ...
            [b[0] for b in INITIAL_BOUNDS],
            [b[1] for b in INITIAL_BOUNDS],
        ).tolist()

        self.dataset_y = []
        for x in self.dataset_x:
            y = run_raptor_step(x[0])
            self.dataset_y.append(y)
            HISTORY_X.append(x)
            HISTORY_Y.append(y)

        self.dataset_x_unit = x_to_unit_arr(self.dataset_x).tolist()
    ...
    def assemble_message(
        ...
        if operation == "initialize_workflow":
            ...
            points_to_predict_unit = x_to_unit_arr(INITIAL_POINTS_TO_PREDICT)
    ...
    def __call__(
        ...
        if operation == "dial.get_next_point":
            ...
            x_suggested_raw = x_from_unit_arr(x_suggested_unit)[0].tolist()
            ...
            best_x, best_y = get_raptor_data_point(x_suggested_raw)

            self.dataset_x.append(best_x)
            self.dataset_y.append(best_y)
            best_x_unit = x_to_unit_arr(best_x).flatten().tolist()
