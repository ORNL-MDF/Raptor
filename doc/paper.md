...
The masking operation consists of a double loop over the number of voxels $n_v$ and the number of segments $n_s$.  To determine the size of the mask at a voxel/segment pair, we must also loop over $n_m$, the number of modes, in order to evaluate the cosine expansion. In the worst case scenario, every voxel is interacting with every segment, which means that we have to perform the cosine expansion during every loop increment. Typically for a 
